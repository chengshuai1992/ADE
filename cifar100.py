...
parser.add_argument('--re', type=float, default='1.0', help="the Regularization coefficient")
parser.add_argument('--classifier_type', type=str, default='AL',
                    help='Which classifier for train. (MCP, AL, ARC, L)')
parser.add_argument('--gpu_id', default=3, type=int, help='id(s) for CUDA_VISIBLE_DEVICES')
...
config["num_classes"] = args.num_class
...
train_dataset = dsets.CIFAR100(root='/disk/hsz/share_win_ubu/GreedyHash-master/data/',
                              train=True,
                              transform=train_transform,
                              download=False)

test_dataset = dsets.CIFAR100(root='/disk/hsz/share_win_ubu/GreedyHash-master/data/',
                             train=False,
                             transform=test_transform)


database_dataset = dsets.CIFAR100(root='/disk/hsz/share_win_ubu/GreedyHash-master/data/',
                                 train=False,
                                 transform=test_transform,
                                 download=False)
...
# load index
test_index = np.load("./data/cifar-100-1wbatches-py/test.npy")
test_data = X[test_index]
test_L = L[test_index]

data_index = np.load("./data/cifar-100-1wbatches-py/database.npy")
data_set = X[data_index]
dataset_L = L[data_index]

train_index = np.load("./data/cifar-100-1wbatches-py/train.npy")
