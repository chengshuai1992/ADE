...
parser.add_argument('--re', type=float, default='0.05', help="the Regularization coefficient")
parser.add_argument('--classifier_type', type=str, default='ARC',
                    help='Which classifier for train. (MCP, AL, ARC, L)')
parser.add_argument('--gpu_id', default=1, type=int, help='id(s) for CUDA_VISIBLE_DEVICES')
...
print("path: ", osp.join(config["output_path"],
...
# Dataset  73257 26032 531131
train_dataset = dsets.SVHN(root='/home/hsz/datasets/SVHN/data',
                           split='train',
                           transform=train_transform,
                           download=False)

test_dataset = dsets.SVHN(root='/home/hsz/datasets/SVHN/data',
                            split='test',
                            transform=train_transform,
                            download=False)


database_dataset = dsets.SVHN(root='/home/hsz/datasets/SVHN/data',
                                 split='extra',
                                 transform=train_transform,
                                 download=False)
...
L = np.array(train_dataset.labels)

test_index = np.load("./data/SVHN-1wnpy/test.npy")
test_data = X[test_index]
test_L = L[test_index]

data_index = np.load("./data/SVHN-1wnpy/database.npy")
data_set = X[data_index]
dataset_L = L[data_index]

train_index = np.load("./data/SVHN-1wnpy/train.npy")
train_data = X[train_index]
train_L = L[train_index]

train_dataset.data = train_data
train_dataset.labels = train_L
test_dataset.data = test_data
test_dataset.labels = test_L
database_dataset.data = data_set
database_dataset.labels = dataset_L
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            if i == len(train_loader) - 2:
                ...
                writer.add_scalars(title, {"loss": running_loss / (i+1),
                                           "loss1": running_loss1 / (i+1),
                                           "loss2": running_loss2 / (i+1)
                                           }, epoch)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            if i % (len(train_dataset) // config["batch_size"]-1) == 0 and i !=0:
